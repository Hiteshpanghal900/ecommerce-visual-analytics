...
\textbf{Cleaning notebook:} \texttt{data\_cleaning/cleaning\_master\_order\_table.ipynb}
...
\begin{itemize}
    \item \texttt{order\_approved\_at} (14 nulls) and \texttt{order\_delivered\_carrier\_date}
    ...
    \item Rows with no \texttt{review\_score} were dropped rather than imputed, since a review
        score cannot be meaningfully guessed from other columns without fabricating signal.
    \item Orders that never reached the customer (no \texttt{order\_delivered\_customer\_date})
        had \texttt{actual\_delivery\_days} and \texttt{delivery\_delay\_days} left null and
        excluded from delivery-timing statistics, rather than being backfilled with a guessed
        delivery date.
\end{itemize}

Engineered columns include \texttt{approval\_delay\_days}, \texttt{shipping\_prep\_days},
\texttt{transit\_days}, \texttt{total\_delivery\_days}, \texttt{delivery\_vs\_estimate\_days},
\texttt{late\_delivery\_flag}, and calendar features (\texttt{purchase\_year},
\texttt{purchase\_month}, \texttt{purchase\_dow}, \texttt{purchase\_hour}). Outliers in numeric
columns were identified via the IQR method and capped (not dropped) into separate
\texttt{\_capped} columns, preserving the original values so that outlier sensitivity could be
checked rather than assumed away. The cleaned table was ultimately split into two
sub-tables -- \texttt{cleaned\_master\_order\_table1.csv} (customer/order/review identity fields)
and \texttt{cleaned\_master\_order\_table2.csv} (order-economics fields: freight, item count,
seller count) -- to keep each table focused.

\section{Customer-Level Master Table}
\textbf{Notebook:} \texttt{data-visualization/customer-master-analysis.ipynb}

Built by aggregating the order table to one row per \texttt{customer\_unique\_id}: total orders,
total/average payment value, average review score, late-delivery rate, product/seller diversity,
and a \texttt{repeat\_customer\_flag}. A known limitation, explicitly documented rather than
...
\textbf{Cleaning notebook:} \texttt{data\_cleaning/clean\_seller\_master\_table.ipynb}

One row per seller (3{,}095 rows, 28 raw columns), aggregating order volume, sales value, delivery
performance, and review behavior. The key missing-value decision: \texttt{avg\_review\_score} is
missing exactly when \texttt{review\_count} is 0 (a seller with no reviews yet). Rather than
imputing a fabricated score, the row is kept and a \texttt{has\_reviews} flag is added, so
...
\texttt{active\_seller\_flag} is \texttt{True} for 100\% of rows and currently carries no
...
\textbf{Cleaning notebook:} \texttt{data\_cleaning/clean\_category\_master\_table.ipynb}
...
the 73 surviving categories have complete, non-imputed \texttt{has\_reviews} and
\texttt{has\_delivery\_data} coverage. Two categories (\texttt{pc\_gamer},
\texttt{portateis\_cozinha\_e\_preparadores\_de\_alimentos}) have no official English translation
and are kept under their Portuguese name -- a labeling quirk only, not a data-quality gap.

\section{Marketing-Funnel Table}
\textbf{Cleaning notebook:} \texttt{data\_cleaning/clean\_marketing\_funnel\_table.ipynb}

One row per marketing-qualified lead (8{,}000 leads), with \texttt{is\_converted\_flag} set when a
matching closed deal exists. Two cleaning decisions stand out:
\begin{itemize}
    \item \texttt{declared\_monthly\_revenue} and \texttt{declared\_product\_catalog\_size} were
        ...
        than a real declared value. These were recoded to \texttt{NaN} with explicit
        \texttt{has\_declared\_revenue} / \texttt{has\_declared\_catalog\_size} flags, leaving only
        45 of 842 converted deals (5.3\%) with a usable, non-placeholder revenue figure.
    \item One converted deal showed a negative \texttt{time\_to\_win\_days} (recorded as won two
        days \emph{before} first contact) -- almost certainly an upstream data-entry error. It was
        flagged via \texttt{time\_to\_win\_valid\_flag} and excluded from timing statistics rather
        than silently kept or dropped from the table entirely.
    \item Deal-side fields (\texttt{business\_segment}, \texttt{lead\_type},
        \texttt{business\_type}, \texttt{sdr\_id}, \texttt{sr\_id}, \texttt{won\_date}) are
        \texttt{NaN} precisely when \texttt{is\_converted\_flag} is \texttt{False}, by construction
...
\textbf{Notebook:} \texttt{EDA/review\_message\_analysis.ipynb}
...
returns/refunds, etc.). This structured output (\texttt{sentiment}, \texttt{primary\_topic}) is the
...
\begin{itemize}
    ...
    \item Late-delivery rate correlates with average review score at $-0.42$ across sellers, and
        with the derived \texttt{review\_risk\_rate} at $+0.40$.
...
\begin{itemize}
    ...
    \item Category size and category satisfaction are essentially uncorrelated ($\sim$0.01) -- a
        ...
        low-risk.
    \item \texttt{office\_furniture} stands out as the most important category to act on: it does
...
\begin{itemize}
    ...
    \item Channel volume and channel quality diverge: \texttt{organic\_search} and
        \texttt{paid\_search} bring the most leads, but \texttt{unknown} (untracked referral) and
        \texttt{paid\_search} convert best; \texttt{social} and \texttt{email} carry real volume
        (300+ leads each) but convert well below average.
    \item Converted leads skew \textasciitilde2:1 toward \texttt{reseller} over
        \texttt{manufacturer} business type, confirming the marketplace's structural role as a
...
This chapter presents the project's central narrative, built in \texttt{combined\_eda.ipynb}: a
...
current \texttt{processed\_customer\_reviews.csv} produced the 9.23\% figure reported here. This
...
\textbf{bed\_bath\_table} has the highest quality-issue rate at \textbf{5.2\%}, followed by
\textbf{luggage\_accessories} (4.8\%) and \textbf{office\_furniture} (4.5\%);
\textbf{furniture\_decor}, \textbf{housewares}, and \textbf{telephony} also sit above 3\%.
...
\texttt{organic\_search} and \texttt{paid\_search} bring in the most leads by volume, but they are
not the channels that convert best -- \texttt{paid\_search} and untracked/\texttt{unknown} referral
sources actually convert at a higher rate, while high-volume channels like \texttt{social} and
\texttt{email} convert well below average. Among sellers who do convert, \textbf{resellers
...
groups identified above. Confirming that link directly (joining converted \texttt{seller\_id}s back
...
\begin{enumerate}[leftmargin=2em]
    ...
    \item \textbf{Targeted seller and category quality program}, not a blanket policy -- audit the
        ...
        (bed\_bath\_table, luggage, furniture, home goods) identified in Section~4.6.
    ...
    \item \textbf{Second-order retention nudge conditioned on a clean first delivery} -- the data
        ...
        good.
    \item \textbf{Revisit acquisition channel mix} -- channels such as \texttt{social} and
        \texttt{email} carry real lead volume but convert well below the funnel average;
...
\begin{itemize}[leftmargin=2em]
    ...
    \item \textbf{Marketing-funnel revenue fields are largely unusable.} 95\% of converted deals in
        the marketing-funnel table have a placeholder (originally 0, recoded to \texttt{NaN})
    ...
    \item \textbf{Known data-quality flag.} \texttt{active\_seller\_flag} in the seller-level table
        is uniformly \texttt{True} and should not be used for active/inactive filtering until fixed
        upstream.
\end{itemize}
...
\begin{itemize}[leftmargin=2em]
    \item All data-cleaning notebooks (\texttt{data\_cleaning/})
    \item All exploratory-analysis notebooks (\texttt{EDA/}, \texttt{data-visualization/})
    \item The combined explanatory narrative notebook, \texttt{combined\_eda.ipynb}, from which
        this report's Chapter~4 is drawn
    \item An interactive Plotly Dash dashboard (\texttt{dashboard/app.py}) for exploring the
        cleaned tables visually
    \item The per-table question-and-answer reference documents (\texttt{questions\_ans\_by\_table/})
    \item \texttt{requirements.txt} for reproducing the Python environment
\end{itemize}

\section{Key Files Referenced in This Report}
\begin{itemize}[leftmargin=2em]
    \item \textbf{Narrative source:} \texttt{combined\_eda.ipynb}
    \item \textbf{Cleaning notebooks:}
        \texttt{data\_cleaning/cleaning\_master\_order\_table.ipynb},
        \texttt{data\_cleaning/clean\_seller\_master\_table.ipynb},
        \texttt{data\_cleaning/clean\_category\_master\_table.ipynb},
        \texttt{data\_cleaning/clean\_marketing\_funnel\_table.ipynb}
    \item \textbf{EDA notebooks:}
        \texttt{EDA/eda\_orders.ipynb}, \texttt{EDA/eda\_master\_orders\_table.ipynb},
        \texttt{EDA/eda\_seller\_master\_table.ipynb},
        \texttt{EDA/marketing-funnel-analysis.ipynb},
        \texttt{EDA/review\_message\_analysis.ipynb},
        \texttt{data-visualization/customer-master-analysis.ipynb},
        \texttt{data-visualization/seller\_product\_analysis.ipynb}
    \item \textbf{Per-table Q\&A reference documents:}
        \texttt{questions\_ans\_by\_table/orders\_table1.md},
        \texttt{questions\_ans\_by\_table/orders\_table2\_questions.md},
        \texttt{questions\_ans\_by\_table/seller\_table\_questions.md},
        \texttt{questions\_ans\_by\_table/customer\_table\_questions.md},
        \texttt{questions\_ans\_by\_table/category\_table\_questions.md},
        \texttt{questions\_ans\_by\_table/marketing\_funnel\_table\_questions.md}
    \item \textbf{Interactive dashboard:} \texttt{dashboard/app.py} (see
        \texttt{dashboard/README.md} for run instructions)
\end{itemize}

\section{Reproducing This Report}
The figures embedded in this report were extracted directly from the executed outputs of
\texttt{combined\_eda.ipynb}. To reproduce them: clone the repository above, install dependencies
from \texttt{requirements.txt}, and execute \texttt{combined\_eda.ipynb} end to end (e.g.\ via
\texttt{jupyter nbconvert --to notebook --execute --inplace combined\_eda.ipynb}); all figures and
